...
def create_label_scaler(train_windows):
    """Create label scaler from training windows (handles both scalar and sequence labels)"""
    from sklearn.preprocessing import StandardScaler
    ...
    # Handle both scalar (old) and sequence (new) labels
    # Flatten sequences if needed
    label_array = []
    for label in train_labels:
        if isinstance(label, (list, np.ndarray)):
            label_array.extend(np.array(label).flatten())
        else:
            label_array.append(label)
    
    scaler = StandardScaler()
    scaler.fit(np.array(label_array).reshape(-1, 1))
...
def apply_label_scaling(windows, scaler):
    """Apply label scaling to windows (handles both scalar and sequence labels)"""
    if scaler is None:
        return windows
    ...
    for window in windows:
        ...
        if new_window.get('stress_label') is not None:
            ...
            # Handle sequence labels (new) vs scalar labels (old)
            if isinstance(original_label, (list, np.ndarray)):
                # Sequence label: scale each timestep
                label_array = np.array(original_label).reshape(-1, 1)
                scaled_array = scaler.transform(label_array).flatten()
                new_window['stress_label'] = scaled_array.astype(np.float32).tolist()
            else:
                # Scalar label: scale single value
...
def main():
    ...
    if model_type == "late_fusion":
        ...
        # Get selected modalities from config
        ...
        # Wrap with adapter (pass selected modalities)
        logger.info(f"Converting data format for late fusion (concatenated -> dict)...")
        logger.info(f"Using modalities: {selected_modalities}")
        train_dataset = LateFusionDatasetAdapter(train_dataset, modality_order=selected_modalities)
        val_dataset = LateFusionDatasetAdapter(val_dataset, modality_order=selected_modalities)
        test_dataset = LateFusionDatasetAdapter(test_dataset, modality_order=selected_modalities)
        
        # Create collate function with modality order
        def collate_fn(batch):
            return late_fusion_collate_fn(batch, modality_order=selected_modalities)
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch in train_loader:
            ...
            # Denormalize for metrics (handle sequence predictions)
            if label_scaler is not None:
                pred_np = predictions.detach().cpu().numpy()
                labels_np = labels_norm.cpu().numpy()
                
                # Handle sequence predictions: [batch_size, window_length]
                if pred_np.ndim == 2 and pred_np.shape[1] > 1:
                    # Sequence prediction: flatten for scaling
                    pred_flat = pred_np.flatten().reshape(-1, 1)
                    labels_flat = labels_np.flatten().reshape(-1, 1)
                    pred_denorm_flat = label_scaler.inverse_transform(pred_flat)
                    labels_denorm_flat = label_scaler.inverse_transform(labels_flat)
                    # Reshape back to sequences
                    pred_denorm = torch.FloatTensor(pred_denorm_flat.reshape(pred_np.shape))
                    labels_denorm = torch.FloatTensor(labels_denorm_flat.reshape(labels_np.shape))
                else:
                    # Scalar prediction (backward compatibility)
                    if pred_np.ndim == 1:
                ...
                pred_denorm = predictions.detach().cpu()
                labels_denorm = labels.cpu()
            
            # Compute MAE: mean over all timesteps and samples
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                # Compute MAE and RMSE: mean over all timesteps and samples
                mae = torch.mean(torch.abs(pred_denorm - labels_denorm)).item()
